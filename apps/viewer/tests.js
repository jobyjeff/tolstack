...
(function (VA) {
  ...
  VA.runTests = async function () {
    ...
    // A refusal, watched happening. `needle` is required to be IN the message
    // whenever one is given, because a bare "it threw" passes on any error at
    // all -- including the TypeError a typo in the check itself would raise.
    // Pass "" for a throw whose message is the JS engine's and not ours.
    function throws(fn, needle, msg) {
      ...
      if (needle) has((err && err.message) || String(err), needle, msg);
    }
    ...
    //
    // Every vocabulary Python owns is read out of apps/viewer/vocab.gen.js
    // (generated by scripts/generate_js_vocabulary.py, compared byte for byte
    // by tests/test_js_vocabulary_is_generated.py). This page holds no copy of
    // one; what it holds is the per-value COPY beside them, and VOCAB.table
    // pairs the two at load.
    //
    // That refusal is what replaced the pytest modules that used to read these
    // tables with a character scanner, so it is the thing to watch working.
    // Synthetic tables rather than the live ones, because the live ones are
    // correct by construction: if they were not, this suite would have thrown
    // while loading viewer.js and no check in it would have run at all.

    await test("the generated vocabulary module refuses a table whose keys " +
      "have drifted from it, and freezes the one it returns", function () {
        var VOCAB = window.TolstackVocab.viewer;
        var words = VOCAB.list("VERDICT_SCOPES");
        ok(words.length >= 2, "the fixture below needs at least two words");

        function entriesFor(names) {
          var out = {};
          names.forEach(function (name) { out[name] = { note: name }; });
          return out;
        }

        // Python emits a word the page has no branch for: the reader would get
        // the loud unlabelled treatment for an ordinary value.
        throws(function () {
          VOCAB.table("VERDICT_SCOPES", entriesFor([words[0]]));
        }, words[words.length - 1], "a missing word must be named in the throw");

        // ...and the other direction: a branch for a state Python cannot emit
        // reads as "this case is handled" to the next author.
        throws(function () {
          VOCAB.table("VERDICT_SCOPES",
            entriesFor(Array.prototype.slice.call(words).concat(["provisional"])));
        }, "provisional", "an impossible word must be named in the throw");

        // A vocabulary this module does not carry at all -- the shape a rename
        // in Python leaves behind once the file is regenerated.
        throws(function () { VOCAB.list("NO_SUCH_VOCABULARY"); },
               "NO_SUCH_VOCABULARY");

        // The returned table is frozen, which is what closes the hole the
        // retired scanner documented and could not see: a key attached from
        // outside the literal, by assignment or by Object.assign.
        var table = VOCAB.table("VERDICT_SCOPES",
          entriesFor(Array.prototype.slice.call(words)));
        throws(function () { table.sneaky = { note: "added later" }; }, "");
        throws(function () { Object.assign(table, { sneaky2: {} }); }, "");
        eq(table.sneaky, undefined, "nothing was added");

        // ...and so is the word list itself.
        throws(function () { words.push("sneaky"); }, "");
        eq(words.indexOf("sneaky"), -1, "nothing was added");
      });
    ...
    // the builder really emits: SIX live entries carry a run_dir, every one of
    // them `resolved_by: "source_ref_export"`, across three drawing-checker
    // runs and four stacks (recounted 2026-09-18 -- this said three entries,
    // which was the count of distinct RUNS). The sweep keeps its
    ...
    function navStudy(verdict, badgeKeys) {
      return {
        id: "s_" + (verdict.state) + "_" + badgeKeys.join("_"),
        title: "Study " + verdict.state, status: "ok", description: null,
        verdict: verdict,
        attention: {
          unverified: [], noTolerance: [], excluded: [],
          badges: badgeKeys.map(function (key) {
            return { key: key, text: VA.ATTENTION[key].text,
                     title: VA.ATTENTION[key].title };
          }),
        },
      };
    }
    ...
    await test("VA.studyNavAlerts folds the attention flags in the order they " +
      ...
      function () {
        ...
        alerts.forEach(function (alert) {
          eq(alert.text, VA.ATTENTION[alert.kind].text);
    ...
    var ALERT_TREE = {
      topologies: [{
        id: "t1", title: "Topology", description: null,
        studies: [
          navStudy(CLEAN, []),
          navStudy(FAILING, ["unverified", "no_tolerance"]),
          navStudy(NO_CRITERION, []),
        ],
      }],
      looseStacks: [],
    };
    ...
    await test("hovering the icon opens the page's own alerts card, and " +
      "clicking it does NOT select the study underneath", function () {
        ...
        var root = render(function (r) {
          VA.renderNavTree(r, ALERT_TREE, { mode: "topology" }, {
            onTopology: function () {},
            onStudy: function (topologyId, studyId) { selected.push(studyId); },
            onCardShow: function (card, trigger) { shown.push([card, trigger]); },
          });
        });
    ...
    // --- which leader style a fresh load draws --------------------------------
    //
    // ANGLED since 2026-09-21 (viewer_nav_alert_badge_and_angled_default).
    // Jeff, having tried both against the real mechanisms: "angled by default
    // (jogged isn't really usable yet and arguably isn't worth putting more
    // effort into since the angled lines look just fine)." Both styles stay on
    // the toggle; apps/viewer/README.md's leaders section carries the decision
    // and the note that jogged is deprioritized.
    await test("the leader default is angled, is a real style, and the " +
      "toolbar names it", function () {
        eq(VA.DEFAULT_LEADER_STYLE, "angled");
        ok(VA.LEADER_STYLES[VA.DEFAULT_LEADER_STYLE],
           "the default must be a style the toggle can cycle to");
        // A preference-less state: the toolbar falls back to the default
        // rather than to whichever style happened to ship first.
        var root = render(function (r) {
          VA.renderTopoToolbar(r, { topologyId: TOPO.id, studyId: null,
            rowDensity: "comfortable" }, TOPO, {});
        });
        var toggle = all(root, "button.tvpick__mode").filter(function (n) {
          return n.getAttribute("id") === "leader-style-toggle";
        })[0];
        ok(toggle, "expected #leader-style-toggle on the toolbar");
        has(toggle.textContent, "Leaders: " + VA.LEADER_STYLES.angled.label);
        // Both are still reachable, in both directions.
        eq(VA.LEADER_STYLES.angled.next, "jogged");
        eq(VA.LEADER_STYLES.jogged.next, "angled");
      });

    // One default, one place. The app shell's initial state and the toggle's
    // own fallback both read VA.DEFAULT_LEADER_STYLE; a literal re-introduced
    // at either site is how a default half-moves, which is exactly what this
    // pass had to undo.
    await test("no leader default is written as a literal in the app shell",
      function () {
        var src = typeof VIEWER_SRC !== "undefined" ? VIEWER_SRC : null;
        if (!src) return;
        var app = src.readText("topology_app.js");
        ok(app, "topology_app.js must be readable");
        eq(/leaderStyle\s*[:=]\s*"/.test(app), false,
           "leaderStyle must be initialised from VA.DEFAULT_LEADER_STYLE, " +
           "not from a quoted style name");
        var view = src.readText("views/topology.js");
        ok(view, "views/topology.js must be readable");
        eq(/VA\.LEADER_STYLES\.jogged/.test(view), false,
           "the toolbar's fallback preset must be the default, not jogged");
      });
