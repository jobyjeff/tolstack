...
| `stack_rotor_fastener_length.json` | **Rotor balance-mass bolt**, 217755 sheet 8 SECTION T-T — a nine-way NAS6403 grip-*selection* family (note 24: "select for correct grip length"), not one fixed dash. Built from scratch; a shadow exercise against Jason Ryan's independent Excel stack (comparison pending the screenshot). |
...
| `hardware_entries.json` | Fastener-library seed: every standard part the stacks consume, each with inline values and a `library_ref` filled exactly where the spec library holds the part (`test_only_the_one_entry_was_promoted` owns how many that is). Now also the two NSK bearings under source-control drawings, the rotor bolt's nine-member NAS6403 grip family plus its MS21299C3 washer, and (2026-09-15) the two RBC link bearings and two NAS77 flanged bushings whose bands are printed in the catalogs already in the spec pile. |
...
| `WORKSHEET_endstop_vision_baseline.md` | **Not a stack — a calibrated capability baseline.** Handoff `endstop_vision_baseline`: an attempt to derive the end-stop stack from the drawings alone, scored against `WORKSHEET_end_stop_graft.md`. Sealed prediction first, then one capped pass. Result: **11 of 43** rows traced, from 6 distinct callouts (§11, handoff `endstop_piece_part_acquisition`, re-scored the last four piece-part drawings plus the two MS14101/MS14103 bearing spec sections against the ten rows still blocked on document acquisition; 35 of 43 now `located`). Carries the finding that **217755 has no dimensions on any of its eight graphical sheets**, that two piece-part drawings explicitly delegate completeness to their 3D model, and the breakage taxonomy that is the requirements input for the 3D-annotation-surface draft. |
...
| `AUDIT_20260915_full_pass.md` | **The first sweep of every stack and topology as one consistency pass** (handoff `stack_fable_audit`). Headline: no transcribed value was wrong anywhere; the defects were missing members (the pitch-link joint gained its eye and flanged bushing), catalog-provable bands sitting `untraced`, and identity bookkeeping (the pitch link is 213862-002). Carries the ranked operator queue with forge todo ids. |
